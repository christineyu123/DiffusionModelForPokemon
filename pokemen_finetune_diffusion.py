...
def train(
    ...
    dataset = datasets.ImageFolder('my_pokemon_folder/images',
    ...
    optimizer = torch.optim.AdamW(image_pipe.unet.parameters(), lr=1e-4)
    ...
    for epoch in range(num_epochs):
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            ...
            clean_images = batch[0]['images'][0].to(device)
            ...
            if (step+1)%save_model_every == 0:
                image_pipe.save_pretrained(model_save_name+f'step')
...
train(image_size=128,batch_size=4,num_epochs=100,
...
log_samples_every=100, save_model_every=100, model_save_name= 'pokemon_model', device = device)
